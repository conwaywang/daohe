...
class CreateRoomHandler(BaseHandler):
    ...
    def get(self):
        ...
        room_desc = self.get_argument("room_desc").strip()
        room_tags = self.get_argument("room_tags").strip()
        
        if not user_id:
            self.response(ERROR_CR_ID)
            return
        if not room_tags:
            self.response(ERROR_CR_TAGS)
            return
        
        #step1  judge user has create room
        if not self.user_dal.is_create_room_able(user_id):
            self.response(ERROR_CR_ROOM_LIMIT)
            return
        #step2 create room
...
class DissolutionRoomHandler(BaseHandler):
    ...
    def get(self):
        ...
        room_id = int(self.get_argument("room_id"))
        if not room_id:
            self.response(ERROR_DR_ROOM_ID)
            return
        #step1 get room
